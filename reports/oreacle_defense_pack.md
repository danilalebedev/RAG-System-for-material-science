...
`smoke_demo_scenarios.py` отдельно проверяет три пользовательских режима без платных LLM/web вызовов и сохраняет
`data/processed/demo_smoke/smoke_report.json`, который попадает в defense bundle.
