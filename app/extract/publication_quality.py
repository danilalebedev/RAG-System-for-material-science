...
def analyze_records(records_dir: Path, issues: IssueCollector) -> dict[str, Any]:
    stats: dict[str, Any] = {
        ...
        "triaged_parse_failed_records": 0,
        "sample_failures": [],
    }
    ...
    for path in sorted(records_dir.glob("*.json")):
        ...
        if status in {"parse_failed_triaged", "malformed_json_triaged"}:
            stats["triaged_parse_failed_records"] += 1
        failure_text = " ".join(
        ...
        if (
            pub_status != "ok"
            or status
            not in {"ok", "ok_repaired", "not_requested", "refused_triaged", "refusal_triaged", "parse_failed_triaged", "malformed_json_triaged"}
        ) and len(sample_failures) < 20:
