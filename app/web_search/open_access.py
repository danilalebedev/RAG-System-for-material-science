...
DEFAULT_USER_AGENT = "OreacleOpenAccessResolver/0.1 (mailto:example@example.com)"
...
class OpenAccessResolver:
    def __init__(self, *, session: requests.Session | None = None, timeout_seconds: int = 15, user_agent: str = DEFAULT_USER_AGENT) -> None:
    ...
    def resolve(self, *, doi: str | None = None, title: str | None = None, year: str | int | None = None) -> OpenAccessResult:
        ...
        for resolver in (self.resolve_unpaywall, self.resolve_openalex, self.resolve_semantic_scholar, self.resolve_europe_pmc):
            ...
            if result.open_access or result.access_status in {"metadata_only", "paywalled"}:
                return result
        return OpenAccessResult(
            title=title or "",
            doi=doi_clean,
    ...
    def resolve_unpaywall(self, *, doi: str, title: str | None, year: str | int | None) -> OpenAccessResult:
        ...
        payload = self._get_json(f"https://api.unpaywall.org/v2/{quote(doi)}", params={"email": "example@example.com"})
        if not payload:
