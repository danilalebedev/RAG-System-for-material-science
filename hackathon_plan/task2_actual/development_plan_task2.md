...
7. GUI:
   - единая форма естественного запроса;
   - фильтры по материалам, процессам, свойствам, диапазонам, географии, источникам;
   - вкладки `Answer`, `Evidence`, `Graph`, `Gaps`, `Sources`, `Benchmarks`;
   - экспорт CSV/JSON/GraphML/PNG/PDF;
   - feedback для экспертной корректировки сущностей/связей.
...
| UI | Streamlit + Plotly/PyVis или Gradio fallback | быстро показать Answer/Evidence/Graph/Gaps/Sources/Benchmarks |
...
1. Сделать Streamlit UI:
   - поле вопроса;
   - фильтры material/process/property/range/geo/source;
   - вкладки `Answer`, `Evidence`, `Graph`, `Gaps`, `Sources`, `Benchmarks`.
2. Подключить query backend через функцию/локальный API.
3. Показать mini-graph через PyVis/Plotly/NetworkX export.
4. Добавить export evidence table в CSV/JSON и graph в GraphML/PNG.
5. Добавить feedback log: исправить entity/relation или отметить неверный ответ.
...
| GUI съест время | Streamlit MVP: только рабочие вкладки и экспорт, без сложного frontend |
...
| E. Query backend / answer composer | понять пользовательский сценарий, переформулировать вопрос, задать уточнение, собрать grounded answer | `app/query/*`, `config/query/*`, `scripts/run_query.py` |
...
| G. GUI / demo application | интерфейс запроса, фильтры, Answer/Evidence/Graph/Gaps/Sources/Benchmarks, экспорт, feedback | `app/ui/*`, `config/ui/*`, `scripts/run_demo.py`, `hackathon_plan/ux/*` |
...
Подробное техническое ТЗ, библиотеки и ссылки на статьи вынесены в `task2_actual/workstream_technical_specs.md`.
