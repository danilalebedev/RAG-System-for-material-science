...
Цель этого документа - разделить разработку так, чтобы параллельная работа не конфликтовала по файлам и данным. Базовое правило: модули читают артефакты предыдущего слоя и пишут только в свою зону `app/*`, `scripts/*` и `data/processed/<module>/`. Общие схемы меняются только через согласованный контракт.
...
flowchart LR
    ...
    CANON --> EXTRACT
    EXTRACT --> FACTS["data/processed/extraction/facts.jsonl<br/>procedure_summaries.jsonl"]
    ...
    USER["User question"] --> QUERY["Query understanding"]
    QUERY --> RAG
...
| C. Metadata extraction + graph | RECIPER summaries, JSON extraction сущностей/отношений, numeric facts, source spans, graph nodes/edges | `app/extract/*`, `app/graph/*`, `scripts/extract_metadata.py`, `scripts/build_graph.py`, `config/extraction/*`, `data/processed/extraction/*`, `data/processed/graph/*` | normalization dictionaries как источник истины; RAG scoring без контракта | `facts.jsonl`, `procedure_summaries.jsonl`, `nodes.jsonl`, `edges.jsonl`, `source_spans.jsonl` |
...
| E. Query and answer UX | обработка вопроса пользователя, intent/slots, уточнения, answer composer, UI сценарии | `app/query/*`, `app/ui/*`, `scripts/run_demo.py`, `config/query/*`, `hackathon_plan/ux/*` | low-level parsers/index builders | `answer`, `evidence pack`, `clarification question`, UI views |
...
6. Query/UI собирает демонстрационные сценарии и answer composer.
7. Evaluation сравнивает B0-B6 по одному набору вопросов.
