...
- `scripts/demo_capability_matrix.py` - no-network audit-карта по главным требованиям демо: RouterAI BGE-M3 indexes,
  GUI contract, web/deep search, PDF/DOCX/ZIP exports, smoke/preflight и pitch/video artifacts.
- `scripts/build_defense_bundle.py` - сборка lightweight ZIP для организаторов: pitch/runbook/storyboard docs, preflight JSON,
  capability matrix, demo smoke JSON, RouterAI BGE-M3 manifests и README с командами запуска без `.env`, raw corpus и full web text.
...
```powershell
.\.venv\Scripts\python.exe scripts\build_defense_bundle.py --run-preflight --run-smoke --run-capability-audit
```
